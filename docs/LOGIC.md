...
- **动画**(`render/route_intro.py`,PIL+ffmpeg):默认 7s(4.2s 被反馈"一下就没了";`ROUTE_INTRO_DURATION_SEC` 可调),轨迹按弧长匀速描画(重采样)+辉光行进头+拍照落点亮点+全程缓推镜头(ken-burns ~10%),底部黑衬条上起终点标题(同前缀合并、超宽自动缩字号)+日期/里程副标,首尾黑场淡入淡出。由 render_video 的 ending 同款转码分支归一后进 concat。

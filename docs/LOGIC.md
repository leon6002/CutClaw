...
- **动画**(`render/route_intro.py`,纯 PIL+ffmpeg,零瓦片零版权):暗色底+淡网格,轨迹按弧长匀速描画(重采样,GPS 点密度天差地别)+辉光行进头+拍照落点亮点,底部起终点标题(同前缀合并、超宽自动缩字号)+日期/里程副标,首尾自带黑场淡入淡出。720p 级生成,由 render_video 的 ending 同款转码分支归一后进 concat。

...
- **聚类**(`src/utils/burst_cluster.py`,纯本地零 API 成本):时间窗(相邻 ≤15s)+ dHash 汉明距离(≤16)双闸门;视觉判定与**簇内任意成员**比(只比最后一张会因中间一张偏差照断链,把一波连拍切碎)。阈值来自长白山实测:15s 窗内换姿势落在 h12-16,窗内转头拍别处 h≥17,窗外相邻对全部 h≥17 → 时间窗别放宽,视觉闸取 16。
- **⚠ Immich v3 坑**:`search/metadata` **不返回 stack 字段**(withStacked 传啥都不带,实测)——堆叠信息必须单独 GET `/stacks` 全列表(列表里 assets 即全量)建 asset→stack 映射再回填,否则子项永远藏不住("堆叠了还是全都在"返工教训)。
- **合并**:重分析把已有封面也纳入(只排除子项);应用时若簇内有成员已是封面,先 DELETE 旧堆叠、其子项并进新簇再建(实测 5 张簇含 5+2 两个旧堆 → 合成 10 张一堆)。
...
- **二期(AI 场景去重,2026-07-11)**:dHash 天花板实锤 —— 同场景换姿势/换构图 h17-41,与真场景切换(h≥17)完全重叠,阈值再放必误伤。本地分析时顺便产出**场景候选组**(模拟应用后的时间线 ≤30s 链分组,每组 ≤12 张);「AI 场景去重」按钮逐组 1 次 VLM 调用(`_SCENE_GROUP_PROMPT`:找"重复可收纳"组+选最佳+一句理由),组结果按**内容签名**缓存(`scene_done`),重跑/重分析零重付,每组落盘断点,失败组不重试下次自动补。UI:簇带 连拍/🧠AI场景 徽章 + AI 理由,按钮标注调用次数上限。

---

## 21. 旅程轨迹开场 + 旁白地理叙事

> 动机(2026-07-11,用户选定"地图 API 应用"方向 1+2):成片开头一段真实 GPS 轨迹动画("我们在哪、走了多远"),旁白从"报地名"升级为"讲旅程"。

- **数据链**(`_build_route_data`,server):shot_point 源视频名 → immich_map → Immich 资产拍摄日(墙钟)→ 同行程日期**全部**带 GPS 资产(照片密度才够画轨迹)→ 稀化(≥60s 或 ≥0.2km)→ **瞬移离群剔除**(隐含时速 >160km/h 的 GPS 漂移点,实测把里程灌水 638→200km)→ 高德起终点地名(网格缓存)→ `route_intro.json` 落盘(1 天内新鲜复用)。
- **底图**(用户反馈"没有地图图层看不出是什么"):高德**静态地图 API**(1024×1024@scale2,项目一次调用随 json 永久缓存,`route_map.png`);无暗色官方样式 → 本地电影化处理(退饱和 0.22 + 压暗 0.42 + 蓝移)。**对位关键**:轨迹点必须先 WGS84→GCJ02 再按 web-mercator(n=256·2^zoom·2)投到底图,否则漂几百米;zoom 自动选到 bbox ≤45% 画幅(任意比例裁剪后都装得下)。没 key/配额错自动回退抽象暗色风格。
- **动画**(`render/route_intro.py`,PIL+ffmpeg):默认 10s(4.2s/7s 均被反馈太快),轨迹按弧长匀速描画(重采样)+辉光行进头+拍照落点亮点+全程缓推镜头(ken-burns ~10%),底部黑衬条上起终点标题(同前缀合并、超宽自动缩字号)+日期/里程副标,首尾黑场淡入淡出。由 render_video 的 ending 同款转码分支归一后进 concat。
- **照片冒泡**:沿累计里程等距挑 ≤6 张,行进头经过时弹出拍立得贴纸(0.5s 过冲缩放,交替 ±6° 旋转);同路段(5% 里程桶)内按红心/评分优先(S-A-B > 未评 > C-D)。**算法认不出"文件翻拍"这类废片**(白纸检测在雪景低饱和场景下失效,实测)→ 预览面板点缩略图手动剔除(`style.bubble_exclude`)。
- **网页预览调参**(`web/src/components/RouteIntroPreview.tsx`):canvas 复刻渲染器关键视觉(含 JS 版 WGS84→GCJ02——不转轨迹对不上底图),时长/亮度滑杆即时生效、冒泡照片点选剔除,「保存参数」写 `style` 进 route_intro.json,正式渲染读同一份(所见即所得);参数在重建路线数据时保留。
- **接线陷阱**(踩过):①旁白 `at_sec` 相对正片计时,intro 前置后必须整体 +intro 时长,否则每句提前;②片头字幕原来判 `i==0`,intro 占位后 title 永不出现 → 改画在第一个**主**镜头(`_first_main_idx`);③开场生成失败只警告不阻塞渲染(锦上添花不许拖垮成片)。
- **旁白**:`/api/narration/generate` 自动带上旅程事实(起终点/天数/里程,`_journey_line`),提示词要求"自然带进某一句,别报数字腔"。
- **UI**:渲染页「🗺 旅程轨迹开场」开关(默认开),旁边实时显示可用性(起终点·里程·点数);无 GPS 数据时渲染任务里记一条已跳过。
- 未做(候选):GPS 地点配额进选材(防 5 个镜头全在同一观景台);Immich 管理页旅程视图;景点级 POI 标签。
...
- **轨迹播放**(多轮防晕迭代后的最终形态):
  - **屏幕恒速**:像素速度锁定(默认 55px/s),每帧物理位移 = 像素速度 ÷ 当前缩放比例 —— 固定物理车速 × 放大镜头 = 屏幕巨快(晕的根源,用户洞察);腿时长由此自然涌现,不预设。
  - **停留/移动分段**:相邻 <300m 聚停留簇(绿点驻留,四宫格分页放映照片,每页 ≤4 张 × photoSec 秒);移动腿 🚗 小车沿路开(开真实路线时沿道路形状),均速 >180km/h 判航段换 ✈️。
  - **镜头挡位制**:连续追踪目标=永远在变焦(晕)→ 每腿最多 3 次换挡:巡航挡(出发定一次,按整腿里程 8/9.5/11)→ 进场挡 12.5(剩 15%/6km)→ 停留挡 14(大景点 14.5);航段 6/9。每次换挡一段完整 flyTo(0.8–2.6s 随跨度),换挡间有**冷却**(默认 ≥8s,4/8/15/30 可选)。
  - **防晕三铁律**:①绝不逐帧改缩放(瓦片加载不完→黑屏 + 全图层逐帧重算→狂抖);②播放中禁止 fitBounds(路线异步加载完成触发的重画会抢镜头);③缩放动画期间矢量层淡出(canvas 渲染器动画中不重绘,线会错位漂移)。
  - **绿色尾迹**:播放中预画路线全部幽灵化(8–15% 透明度),走过的路由绿色辉光尾迹点亮;停止播放恢复全量显示。
  - **⚙ 镜头参数面板**:车速/每页秒数/全部挡位数值可配置,改动即时生效(含播放中),localStorage 持久化,可一键恢复默认。
  - 地图瓦片默认**原色**(压暗滤镜被用户否决,改为 ☀️/🌙 开关)。
- **图层**:标准矢量 / 🛰 卫星(style=6 影像 + style=8 透明路网标注)。均为高德**公开瓦片**(webrd/webst.is.autonavi.com),不走 API key 零配额,浏览器 HTTP 缓存;坐标系同为 GCJ-02。
- **🛣 真实路线**(手动开关):移动腿均速 >180km/h 判**航段**(虚线直连,零调用);否则调高德**驾车路径规划**吸附到真实道路(起终点 + ≤6 个沿途 GPS 途经点保证贴近实际走法)。每腿 1 次调用(走 key 配额),结果按 GCJ 坐标 3 位小数签名**永久缓存** `Output/asset_index/amap_routes.json` —— 同段路全库只调一次。播放时 🚗 沿道路形状开、航段换 ✈️。已知局限:跨夜长间隔会稀释均速,真飞机腿可能被判成自驾(仍能画出路线,只是多耗 1 次调用)。
